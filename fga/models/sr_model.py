...
class SRModel_(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRModel_, self).__init__(opt)
        ...
        if load_path is not None:
            ...
            if self.param_key == 'None':
                self.param_key = None
            self.load_network(self.net_g, load_path, self.opt['path'].get('strict_load_g', True), self.param_key)

        # data normalization
        try:
            self.data_range = self.opt['data_range']
        except KeyError:
            self.data_range = 1
