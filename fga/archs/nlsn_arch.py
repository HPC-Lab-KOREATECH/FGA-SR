# reference: https://github.com/HarukiYqM/Non-Local-Sparse-Attention/blob/main/src/model/nlsn.py
...
class NonLocalSparseAttention(nn.Module):
    ...
    def flops(self, h, w):
        """rough MACs of one NLSA block"""
        flops = 0
        c   = self.conv_match[0].in_channels
        cr  = self.conv_match[0].out_channels           # = c // reduction
        L   = h * w
        # 2 x Conv
        flops += conv_flops(h, w, c,  cr, 3)        # conv_match
        flops += conv_flops(h, w, c,  c, 1)        # conv_assembly (1×1)

        # hash bucket & sparse attention (dominant part)
        # ------------------------------------------------
        buckets     = min(L // self.chunk_size + (L // self.chunk_size) % 2, 128)
        num_chunks  = (L + self.chunk_size - 1) // self.chunk_size
        # similarity: x_att(…,C) · x_match(…,C)
        flops += self.n_hashes * num_chunks * self.chunk_size * (self.chunk_size * 3) * c
        # aggregation: score(…,s) · y_att(…,C/reduction)
        flops += self.n_hashes * num_chunks * self.chunk_size * c * self.reduction
        return flops
...
class NonLocalAttention(nn.Module):
    ...
    def flops(self, h, w):
        """
        Args:
            h, w (int): feature-map 높이·너비
            b (int)   : batch size
        Returns:
            int       : FLOPs (MACs×2) for one NonLocalAttention block
        """
        flops = 0
        c  = self.conv_match1[0].in_channels          # in_channels
        cr = self.conv_match1[0].out_channels         # = c // reduction
        L  = h * w                                    # spatial length

        # ── 1) 1x1 Convs ─────────────────────────────
        flops += conv_flops(h, w, c,  cr, 1)      # conv_match1
        flops += conv_flops(h, w, c,  cr, 1)      # conv_match2
        flops += conv_flops(h, w, c,   c, 1)      # conv_assembly

        # ── 2) Self-Attention MatMuls ────────────────
        #   score =  (L × cr) · (cr × L)   →   L × L
        flops += L * cr * L
        #   x_final = (L × L) · (L × c)    →   L × c
        flops += L * L * c
...
# Non Local Sparse Attention Network (NLSN), patch size 48x48, batch size 16, learning rate 1e-4, rgb range 1, DIV2K dataset, using chop
...
class NLSN(nn.Module):
    ...
    def flops(self, h, w, b=1):
        ...
        # body
        for blk in self.body[:-1]:               # 마지막 conv 전까지
            if isinstance(blk, NonLocalSparseAttention):
                flops += blk.flops(h, w)
            else:   # ResBlock
                flops += blk.flops(h, w)
